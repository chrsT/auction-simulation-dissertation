...
class AgentList(collections.UserList):
    ...
    def gene_score(self, key):
        stats = []
        dove = True if key in ["rep_weighting","personal_experience", "alturistic_punishment"] else False
        for a in self:
...
class Agent(object):
    ...
    def CLASS_generate_random_agent(self):
        genome_dict = { "strategy" : RANDOMS.binary_random_decision(PARAMETERS.INITIAL_HAWK,"H","D"),
                        ...
                        "personal_experience" : 0, ##Disabled for experiment 3.
                        "alturistic_punishment" : RANDOMS.random_int(1,1023)}
    ...
    def new_transaction(self, transaction, feedback_value=[]):
        self.transaction_list.append(transaction)
        for f in feedback_value:
            self.feedback_list.append(f)
    ...
    def AUT_alturistic_punishment(self):
        raise NotImplementedError
...
class DoveAgent(Agent):
    ...
    def get_personal_feedback(self,other_agent):
        ...
        for t in self.transaction_table.get(other_agent.ID,[]):
            feedback = t.other_agent_feedback(self)
            for f in feedback:
                if f is None:
                    continue
                if f.source == ENUMS.FeedbackSource.punishment:
                    continue
    ...
    def AUT_alturistic_punishment(self):
        alturistic_punishment = int(self.get_gene("alturistic_punishment"))

        return RANDOMS.binary_random_decision(alturistic_punishment,True,False)
